...
const Chat = ({ officerId }) => {
  const [messages, setMessages] = useState([]);
  const [input, setInput] = useState("");

  // Get user ID from session storage
  const userId = sessionStorage.getItem("uID");
  const chatId = `${officerId}_${userId}`; // Unique chat ID combining user and officer IDs
  ...
  const handleSend = () => {
    if (input.trim()) {
      ...
      socket.emit("sendMessage", { chatId, sender: "user", message: input });
